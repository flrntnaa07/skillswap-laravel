...
@section('content')
    <div class="px-6 py-8 max-w-2xl mx-auto w-full">
        <h1 class="text-2xl lg:text-3xl font-extrabold text-white">Tambah Skill</h1>

        <form method="POST" action="{{ route('skills.store') }}" class="mt-6 ss-glass ss-card-hover rounded-2xl p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <label class="flex items-center gap-3 ss-glass rounded-xl px-4 py-3 cursor-pointer border border-cyan-300/30 hover:border-cyan-300/60 transition has-[:checked]:border-cyan-300 has-[:checked]:bg-cyan-400/10">
                        <input type="radio" name="skill_type" value="owned" required {{ old('skill_type') == 'owned' ? 'checked' : '' }} class="accent-cyan-400">
                        <div>
                    ...
                    <label class="flex items-center gap-3 ss-glass rounded-xl px-4 py-3 cursor-pointer border border-cyan-300/30 hover:border-cyan-300/60 transition has-[:checked]:border-cyan-300 has-[:checked]:bg-cyan-400/10">
                        <input type="radio" name="skill_type" value="needed" required {{ old('skill_type') == 'needed' ? 'checked' : '' }} class="accent-cyan-400">
                        <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" required
                          class="mt-1 w-full rounded-md border border-cyan-300/30 bg-white/5 px-3 py-2 text-white placeholder-cyan-200/40 focus:ring-2 focus:ring-cyan-300 focus:border-cyan-300 outline-none">{{ old('description') }}</textarea>
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2 rounded-xl text-white ss-btn-primary font-semibold">Simpan</button>
            </div>
        </form>
    </div>
@endsection
